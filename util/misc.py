...
def save_model(config, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('checkpoint-%s-%s.pth' % (epoch_name,config["method_name"]))]
        ...
        model.save_checkpoint(save_dir=config["output_dir"], tag="checkpoint-%s" % epoch_name, client_state=client_state)
